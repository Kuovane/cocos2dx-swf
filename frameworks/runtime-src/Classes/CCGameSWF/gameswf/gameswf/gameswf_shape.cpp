...
//		memcpy(&m_coords[0], coords, coord_count * sizeof(coords[0]));
	...
	void	mesh_set::displayEx(float &w, float &h)
		// Throw our meshes at the renderer.
	{
		assert(m_error_tolerance > 0);
		...
		for (int j = 0; j < m_layers.size(); j++) {
			...
			// Dump meshes into renderer, one mesh per style.
			for (int i = 0; i < l.m_meshes.size(); i++) {
				auto mesh = l.m_meshes[i];
				if (l.m_meshes[i]) {

					 minx = mesh->m_triangle_strip[0];
					 miny = mesh->m_triangle_strip[1];

					 maxx = minx;
					 maxy = miny;
					int len = mesh->m_triangle_strip.size();
					for (int k = 0; k < len; k= k+2)
					{
						auto x = mesh->m_triangle_strip[k];
						auto y = mesh->m_triangle_strip[k+1];
						if (x < minx)
						{
							minx = x;
						}
