...
//		assert(m_current_root.get_ptr() != NULL);
	...
	// Load the actual movie.
	{
		...
		movie_def_impl* mdp = (movie_def_impl*)create_movie(infile.c_str());
		gameswf::movie_definition*	md = mdp;
		if (md == NULL)
		{
		...
		gameswf::root*	m = mdp->create_instance();
		if (m == NULL)
		{
		...
		mdp->saveToFiles(filePath,infile);
