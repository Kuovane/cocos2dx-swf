...
namespace gameswf
{
	...
	struct player : public ref_counted
	{
		...
		typedef std::function<void(unsigned char*pData, std::string sFileName, int w, int h, bool bNeedFree)> WriteBitmapFunc;

		WriteBitmapFunc m_wirteBitmapFunc = nullptr;
		...
		root*  load_file( const char* filePath,  const char* filename);
		bool drawFrame();
		bool isDrawFameEnd();
		void setWriteBitmapFunc(WriteBitmapFunc func);
		movie_def_impl* m_mdp = nullptr;
		int m_curFrame = 0;
