...
//		assert(m_current_root.get_ptr() != NULL);
	...
	root* player::load_file(const char* filePath, const char* filename)
	// Load the actual movie.
	{
		// = filePath;
		std::string infile = filename;
		m_mdp = (movie_def_impl*)create_movie(infile.c_str());
		gameswf::movie_definition*	md = m_mdp;
		if (md == NULL)
		{
			//fprintf(stderr, "error: can't create a movie from '%s'\n", infile.c_str());
			CCLOG("error: can't create a movie from '%s'\n", infile.c_str());
			return NULL;
		}

		gameswf::root*	m = m_mdp->create_instance();
		if (m == NULL)
		{
			//fprintf(stderr, "error: can't create movie instance\n");
			CCLOG("error: can't create movie instance\n");
			return NULL;
		}
		if (m_wirteBitmapFunc)
		{
			m_mdp->setWriteBitmapFunc(m_wirteBitmapFunc);
		}
		//m_mdp->saveToFiles(filePath,infile);
		m_mdp->drawBegin(filePath, infile);
		int	movie_version = m->get_movie_version();

		CCLOG("Playing %s, swf version %d\n", infile.c_str(), movie_version);

		return m;
	}

	bool player::drawFrame()
	{
		bool ret = false;
		int size = m_mdp->m_playlist.size();
		if (m_curFrame < size)
		{
			ret = m_mdp->drawFrame(m_curFrame);
			m_curFrame++;
			if (m_curFrame>= size)
			{
				m_mdp->drawEnd();
	...
	bool player::isDrawFameEnd()
	{
		if (m_mdp && m_curFrame < m_mdp->m_playlist.size())
		{
			return false;
		}
		else
	...
	void player::setWriteBitmapFunc(player::WriteBitmapFunc func)
	{
		m_wirteBitmapFunc = func;
	}
