...
//				printf("signal for frame %d\n", m_loading_frame);
	...
	struct movie_def_impl : public movie_definition_sub
	{
		...
		void initFiles(std::string sOutPath, std::string);
		void closeFiles();
		void writeStrToFile(std::string sStr);
		void removeDepth(int depth,int id);
		void pushMovie(int depth,int characterId);
		void popMovie(int depth,int characterId);
		std::string  getFileName(int id);
		void writeBitmapData(gameswf::bitmap_info* bitmap, int id, int w, int h);
		void writeImageData(unsigned char* pData,std::string fileName, int w, int h,bool flipY,bool bNeedFree=false);
		void setWriteBitmapFunc(std::function<void(unsigned char*, std::string, int, int, bool bNeedFree )> func) { m_writeBitmapFunc = func; };
		void saveToFiles(std::string sOutPath, std::string);
		void resetDraw();
		void drawBegin(std::string sOutPath, std::string);
		bool drawFrame(int curFrame);
		void drawEnd();
		
		void drawMovieClip( bool, matrix&, bool, cxform&,int depth, int clip_depth);
		void drawBitmap(gameswf::bitmap_info*, bool, matrix&,bool, cxform&,rect& sRect,int depth, int clip_depth);
		std::map<int, rect>& getRects() { return s_sRects; }
		std::map<int, bitmap_info*>& getBitmapInfos();
		std::map<int, bitmap_info*>& getCHBitmapInfos();
		int getMovieClipByDepth(int nDepth) ;
		std::map<int, int>& getMovieClips() { return s_movieClips; };
		void addBitmap(int depth, int id, bitmap_info* bitmapinfo);
		int & getBitmapId() { return m_bitmapId; }


		FILE *m_pInfoFile = nullptr;
		//FILE *m_pPinFile = nullptr;
		//FILE *m_pPngFile = nullptr;
		std::string m_sAssetsPath;
		std::string m_sFileName;
		std::map<int, rect> s_sRects;
		std::map<int, gameswf::bitmap_info*> s_sBitmapInfos;
		std::map<int, gameswf::bitmap_info*> s_sChBitmapInfos;
		std::map<int, int >s_movieClips;
		int m_bitmapId = 1;
		std::function<void(unsigned char*, std::string, int, int, bool bNeedFree )> m_writeBitmapFunc = nullptr;
	};
