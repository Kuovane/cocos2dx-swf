...
struct bitmap_info_ogl : public gameswf::bitmap_info
{
	...
	virtual unsigned int getTextureId() { return m_texture_id; };
	// get byte per pixel
...
const char* ColorVertexShader = "\
                                          ...
                                          void main(void) \n\
{ \
    gl_Position =  projection * u_MVPMatrix * vec4(a_position,0.0,1.0);\
}\
                                          ";
...
varying vec2 v_texCoord;\n \
	...
	uniform	vec4 u_MultColor;				\n\
                                              void main(void)\
{\
	 gl_FragColor = texture2D(u_texture,v_texCoord)*u_MultColor + u_AddColor;\
}\
                                              ";

enum {
	kVertexAttrib_Position = 1,
	kVertexAttrib_Color,
...
struct render_handler_ogles : public gameswf::render_handler
{
	...
	static unsigned int u_AddColor;
	static unsigned int u_MultColor;
	...
	mutable bool m_bGenImageMode = false;
		...
		m_display_height(0)
	{
		...
		//m_TextureProgram =  MyUtils::createGLProgram("../res/shaders/texture.vert", "../res/shaders/texture.frag");//BuildProgram(2);
		m_TextureProgram = BuildProgram(2);
		//
		u_AddColor = glGetUniformLocation(m_TextureProgram, "u_AddColor");
		u_MultColor = glGetUniformLocation(m_TextureProgram, "u_MultColor");
	...
	{
		static auto sSize = Director::getInstance()->getOpenGLView()->getFrameSize();
		float w = mat[3];
		float h = mat[7];

		CCLOG("ApplyOrtho %f %f", sSize.width, sSize.height);

		// view(1.0);
		glm::mat4 view = glm::lookAt(glm::vec3(0.0f, 0.0f, -1.0f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, -1.0f, 0.0f));
		view = glm::scale(view, glm::vec3(2.0 / (  sSize.width), 2.0 / ( sSize.height), 1.0f));
		view = glm::translate(view, glm::vec3(-sSize.width /2.0f +w, -sSize.height /2.0f + (sSize.height - h) , 0.0f));

		if (m_currentProgram == m_ColorProgram) {
			glUniformMatrix4fv(projectionUniformC, 1, GL_FALSE, glm::value_ptr(view));
		}
		else if (m_currentProgram == m_TextureProgram) {
			glUniformMatrix4fv(projectionUniformT, 1, GL_FALSE, glm::value_ptr(view));
		}
	...
	void ApplyMatrix(const gameswf::matrix& m)
	{
		...
		mat[5] = m.m_[1][1];
		mat[10] = 1;
		mat[12] = m.m_[0][2]/20.0f;
		mat[13] = m.m_[1][2]/20.0f;
		mat[15] = 1;
	...
	std::map<unsigned int, unsigned int> m_sMapId;
	...
	unsigned int getVAOId(const void* coords, int vertex_count)
	{
			...
			{
				auto ite = m_sMapTextrueId.find(id);
				if (ite != m_sMapTextrueId.end())
				{
					return ite->second;
				}
			}
...

		{
			...
			glBindVertexArray(vertexArrayId);//��VAO�󣬺���glVertexAttribPointer���Զ��洢��VAO��
			glBindBuffer(GL_ARRAY_BUFFER, vertexBufferId);

			if (m_currentProgram == m_TextureProgram)
			{
				GLfloat* tcoord = new GLfloat[4 * vertex_count];
				float *vcoord = (float*)coords;
				auto pS = m_current_styles[LEFT_STYLE].pS;
				auto pT = m_current_styles[LEFT_STYLE].pT;
				for (int i = 0, j = 0; i < 2 * vertex_count; i = i + 2, j = j + 4)
				{


					tcoord[j + 2] = vcoord[i] * pS[0] + vcoord[i + 1] * pS[1] + pS[3];
					tcoord[j + 3] = vcoord[i] * pT[0] + vcoord[i + 1] * pT[1] + pT[3];
					tcoord[j] = vcoord[i];
					tcoord[j + 1] = vcoord[i + 1];
				}


				glBufferData(GL_ARRAY_BUFFER, vertex_count * 4 * 4, tcoord, GL_DYNAMIC_DRAW);
				glVertexAttribPointer(kVertexAttrib_Position, 2, GL_FLOAT, GL_FALSE, 8 * 2, (void*)0);
				glVertexAttribPointer(kVertexAttrib_TexCoords, 2, GL_FLOAT, GL_FALSE, 8 * 2, (void*)8);
				...
				delete[] tcoord;
					...
					m_sMapTextrueId[id] = vertexArrayId;
			}
			else
			{
				...
				{
					tcoord[i] = vcoord[i];
					tcoord[i + 1] = vcoord[i + 1];
				}

				glBufferData(GL_ARRAY_BUFFER, vertex_count * 2 * 4, coords, GL_DYNAMIC_DRAW);
				glVertexAttribPointer(kVertexAttrib_Position, 2, GL_FLOAT, GL_FALSE, 8, (void*)0);
				glEnableVertexAttribArray(kVertexAttrib_Position);
				delete[] tcoord;
				m_sMapId[id] = vertexArrayId;
			}
			//return 0;
...
#else
		{
			...
			if (m_currentProgram == m_TextureProgram)
			{
				...
				for (int i = 0, j = 0; i < 2 * vertex_count; i = i + 2, j = j + 4)
				{
					...
					tcoord[j + 2] = vcoord[i] * pS[0] + vcoord[i + 1] * pS[1] + pS[3];
					tcoord[j + 3] = vcoord[i] * pT[0] + vcoord[i + 1] * pT[1] + pT[3];
					tcoord[j] = vcoord[i]/20.0f;
					tcoord[j + 1] = vcoord[i + 1]/20.0f;
				}
			...
			else
			{
				...
				for (int i = 0; i < 2 * vertex_count; i = i + 2)
				{
					tcoord[i] = vcoord[i]/20.0f;
					tcoord[i + 1] = vcoord[i + 1]/20.0f;
				}

				glBufferData(GL_ARRAY_BUFFER, vertex_count * 2 * 4, (float*)tcoord, GL_DYNAMIC_DRAW);
				glVertexAttribPointer(kVertexAttrib_Position, 2, GL_FLOAT, GL_FALSE, 8, (void*)0);
				glEnableVertexAttribArray(kVertexAttrib_Position);
				delete[] tcoord;
			}
		...
		// Helper for draw_mesh_strip and draw_triangle_list.
	{
		...
		//CHECK_GL_ERROR_DEBUG();
		unsigned int vaoId = getVAOId(coords, vertex_count);
		if (vaoId > 0)
		{
		...
		if (m_currentProgram == m_ColorProgram) {
			...
			m_current_styles[LEFT_STYLE].apply_second_pass(m_bGenImageMode);
			glDrawArrays(primitive_type, 0, vertex_count);
...
unsigned int render_handler_ogles::u_AddColor = 0;
unsigned int render_handler_ogles::u_MultColor = 0;
...
void bitmap_info_ogl::layout(GLuint eMode)
{
	//    return;
